...
<script type="text/javascript">
	var start_url = "{{ URL::route('indexation-urls', [$crawl->id, 'all-urls']) }}";
	var oTable = Ajax.detailDataTable(start_url);

	$('.{{$stat->typ}}').on("click", function() {
		var typ = $(this).attr('class').urlize();;
		var description = $(this).attr('id').urlize();
		var url = laroute.route(typ+'-urls', {crawl:crawl_id, description:description});
		
		oTable.fnReloadAjax(home_url + url);
		$('#detail-title').text(description.titlerize());
	});
	
	// Compare added data tables
